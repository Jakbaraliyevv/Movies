...
function Card_movie() {
      ...
      <section className="text-[#FFF]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-5 lg:gap-6">
          {[...Array(12)].map((_, index) => (
            <div key={index} className="w-full h-auto">
              <div className="w-full">
                <div className="w-full h-[240px] sm:h-[280px] md:h-[300px] lg:h-[320px] xl:h-[340px] bg-gray-300/20 rounded-lg overflow-hidden shadow-md animate-pulse">
                  <div className="w-full h-full bg-gradient-to-r from-gray-300/10 via-gray-300/20 to-gray-300/10 animate-pulse"></div>
                </div>
              </div>

              <div className="flex flex-col gap-2 md:gap-3 items-start p-2">
                <div className="w-full">
                  <div className="h-5 md:h-6 bg-gray-600/30 rounded mb-2 animate-pulse w-3/4"></div>
                  <div className="h-3 md:h-4 bg-gray-600/30 rounded animate-pulse w-full"></div>
                </div>

                <div className="w-full h-[28px] md:h-[30px] bg-[#ffd700]/30 rounded-[5px] animate-pulse"></div>
              </div>
            </div>
          ))}
        </div>
  ...
  // Faqat 12 ta filmni olish (6 tali ekranlarda 2 qator)
  const displayedMovies = data
    .filter((value) => !value?.isPremiere)
    .slice(0, 12);
        ...
        {displayedMovies.map((value) => (
          <div key={value?._id} className="relative w-full h-auto">
            <div className="w-full">
              <img
                onClick={() => navigate(`/movies/${value?._id}`)}
                className="w-full cursor-pointer h-[240px] sm:h-[280px] md:h-[300px] lg:h-[320px] xl:h-[340px] object-cover rounded-lg overflow-hidden shadow-md transition-transform duration-300 hover:scale-105"
                src={value?.posterUrl}
                alt={value?.title}
              />
              <div className="eyeCard">
                <div className="flex items-center gap-1">
                  <Eye
                    size={16}
                    className="text-[#FFF] sm:w-[18px] sm:h-[18px] md:w-[19px] md:h-[19px]"
                  />
            ...
            <div className="flex flex-col gap-2 md:gap-3 items-start p-2">
              <div>
                <h3 className="text-base sm:text-lg md:text-[19px] font-bold line-clamp-1">
                  {value?.title}
                </h3>
                <p className="text-xs sm:text-[13px] md:text-[14px] font-medium text-gray-400 line-clamp-2">
                  {truncateTitle(value?.description, 4)}
                </p>
              </div>
              ...
              <button
                onClick={() => navigate(`/movies/${value?._id}`)}
                className="filmBtn w-full bg-[#ffd700] text-[#000] text-xs sm:text-[13px] md:text-[14px] font-medium rounded-[5px] h-[28px] md:h-[30px] flex items-center justify-center gap-1"
              >
                <Play size={12} className="sm:w-[13px] sm:h-[13px]" />
                <span className="hidden sm:inline">Tomosha qilish</span>
                <span className="sm:hidden">Tomosha</span>
              </button>
            </div>
          </div>
        ))}
