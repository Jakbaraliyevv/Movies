...
function Filter_catagory() {
  const { slug } = useParams(); // ✅ URL dan slug olish
  ...
  if (loading) {
    return (
      <section className="w-[90%] mt-[40px]  m-auto text-[#FFF]">
        {/* Loading indicator */}
  ...
  const displayedMovies = data?.filter(
    (value) => value.category === slug && !value?.isPremiere
  );

  return (
    <section className="mt-[40px] w-[90%] m-auto text-[#FFF]">
      <div className="grid grid-cols-4 gap-[24px]">
...
export default Filter_catagory;
